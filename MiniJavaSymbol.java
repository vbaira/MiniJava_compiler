...
class MiniJavaSymbol{
	/*symbol's name,category(variable or method) and type(type for variable,return type for method).
	  If its a method ,list for parameters */
	private String name;
	...
	private Map<String,MiniJavaSymbol> methodVarMap; //map for local variables of method
	private int offset;
	private String vtableString;



	public MiniJavaSymbol(String n,String c,String t,List<String> pl,int offs){
		name=n;
		category=c;
		type=t;
		paramList=pl;
		...
		if (category.equals("METHOD")){
			/*fill method's Variable Map with parameters from parameter list*/
			methodVarMap=new LinkedHashMap<String,MiniJavaSymbol>();
			if (paramList!=null){
				for (String param : paramList){
					/*check for duplicate names in parameter list*/
					...
					MiniJavaSymbol method_param=new MiniJavaSymbol(token[1],"M_PAR",token[0],null,0);
	...
	public Map<String,MiniJavaSymbol> getMethodMap(){
		return methodVarMap;
	}
	...
	/*insert local variable to a method*/
	public void insertLocalVar(String n,String t){
		/*no duplicate local vars allowed*/
		if (methodVarMap.containsKey(n)){
			throw new RuntimeException("Redeclaration of local variable:"+n+" of method "+name);
		}
		MiniJavaSymbol local_var=new MiniJavaSymbol(n,"M_VAR",t,null,0);
		methodVarMap.put(n,local_var);
	}

	/*return a string containing the types of method's parameters*/
	...
	/*used to compare method prototypes(to check if a method is overriden properly)*/
	public boolean methodIsEqual(MiniJavaSymbol comp){
	...
	public void print(){
		if (paramList==null){
			if (category.equals("METHOD")){
				...
				for (Map.Entry<String,MiniJavaSymbol> local_var : methodVarMap.entrySet()){
					System.out.print("\n\t");
	...
	/*Comparator for sorting MiniJavaSymbol by offset*/
    public static Comparator<MiniJavaSymbol> offsetComparator = new Comparator<MiniJavaSymbol>() {

		public int compare(MiniJavaSymbol m1, MiniJavaSymbol m2) {
