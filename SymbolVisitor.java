...
/*
Second Visitor.
Fill each class' symbol Map with fields and methods declared in it
as well as fields and methods inherited from parent classes.
Also for each method fill its Variable Map with parameters and
variables declared in it
*/
public class SymbolVisitor extends GJDepthFirst< String,String> {
	private static String currentClass;
	private MiniJavaSymbol currentMethod;	
	...
	*/
	public String visit(MainClass n, String argu) {
		...
		String ret_type="public static void";
		String method_name="main";
		List<String> param_list= new LinkedList<String>();
		param_list.add("String[] "+param_name);
		MiniJavaSymbol sym=new MiniJavaSymbol(method_name,"METHOD",ret_type,param_list,0);
	...
	*/
	public String visit(ClassExtendsDeclaration n, String argu) {
		...
		Set<Map.Entry<String,List<MiniJavaSymbol>>> entry_set=parent_sym_map.entrySet();
		for (Map.Entry<String,List<MiniJavaSymbol>> entry: entry_set){
			List<MiniJavaSymbol> value=entry.getValue();
			for( MiniJavaSymbol inherited_symbol : value){
				if ( !(inherited_symbol.getCategory().equals("METHOD")) ){
					...
					if ( !sym_map.contains(inherited_symbol.getName()) ){
						...
						if (entry.getKey().contains(".")){
							key=entry.getKey();
							MiniJavaSymbol sym=new MiniJavaSymbol(inh_field_name,inherited_symbol.getCategory(),inherited_symbol.getType(),inherited_symbol.getParamList(),inherited_symbol.getOffset());
			...
			for( MiniJavaSymbol inherited_symbol : value){
				if ( (inherited_symbol.getCategory().equals("METHOD")) ){
					...
					if ( !sym_map.contains(inherited_symbol.getName()) ){
						...
						if (entry.getKey().contains(".")){
							key=entry.getKey();
							MiniJavaSymbol sym=new MiniJavaSymbol(inh_method_name,inherited_symbol.getCategory(),inherited_symbol.getType(),inherited_symbol.getParamList(),inherited_symbol.getOffset());
							sym_map.insert(key,sym);
						}
						else{
							key=parentClassName+"."+inherited_symbol.getName();
					...
					check if it belongs to field or method or both*/
						...
						List<MiniJavaSymbol> sym_list=sym_map.get(inherited_symbol.getName());
						boolean method_exists=false;
						MiniJavaSymbol symbol=null;
						for( MiniJavaSymbol sym : sym_list){
	...
	*/
	public String visit(MethodDeclaration n, String argu) {
		...
		if ( !ret_type.equals("int[]") && !ret_type.equals("boolean") && !ret_type.equals("int") ){
		...
		if (sym_map.contains(method_name)){
			List<MiniJavaSymbol> sym_list=sym_map.get(method_name);
			for (MiniJavaSymbol s : sym_list){
		...
		if(paramList!=null){
			...
			sym=new MiniJavaSymbol(method_name,"METHOD",ret_type,pl,sym_map.getMethodOffset());
			//System.out.println(paramList);
		}
		else{
			//no parameters->null paramList
			sym=new MiniJavaSymbol(method_name,"METHOD",ret_type,null,sym_map.getMethodOffset());
		}
		sym_map.insert(method_name,sym);
	...
	private static boolean methodOverrrides(MiniJavaSymbol method){
		...
		Set<Map.Entry<String,List<MiniJavaSymbol>>> parent_entry_set=parent_sym_map.entrySet();
		for (Map.Entry<String,List<MiniJavaSymbol>> parent_entry: parent_entry_set){
			List<MiniJavaSymbol> value=parent_entry.getValue();
			/*for each method of parent*/
			for( MiniJavaSymbol inherited_symbol : value){
				if ( (inherited_symbol.getCategory().equals("METHOD")) ){
	...
	public String visit(VarDeclaration n, String argu) {
		...
		/*var declarations inside CLASS BODY*/
			...
			if (sym_map.contains(id)){
				...
				/*everything OK*/
				MiniJavaSymbol sym=new MiniJavaSymbol(id,"F_VAR",type,null,sym_map.getFieldOffset());
